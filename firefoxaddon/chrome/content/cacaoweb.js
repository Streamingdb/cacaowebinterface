...
var cacaoweb = function () {
	...
	*/
	function init() {
		...
		setTimeout(function () {
				var isrunningscript = content.document.getElementById('isrunningscript');
				if (!isrunningscript) { // tests if the cacaoweb API is already running in the page, if that's the case there is no need for the following
					...
					if (content.document.body) {
						var scriptblock = content.document.getElementById('isrunning'); 
						if (scriptblock) { // TODO: it means the isrunning script has already been inserted by someone??
							...
							var docscript = content.document.createElement("script");
							...
							content.document.body.appendChild(docscript);
	...
	function putmarker() {
		...
		div.id = 'cacaowebfirefoxextension';
	...
	*/
	...
	function findvideos() {		
		var cacaodiv = content.document.getElementById("cacaorunning"); 
		...
		if (isVideobblink(loc) || isVideozerlink(loc) || isMixturelink(loc) || isPutlockerlink(loc) || 
			...
			var docs = getDocuments(content.document.defaultView);
